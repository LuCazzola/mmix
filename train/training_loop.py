...
from sample.mix import main as mix
import copy
...
class TrainLoop:
    def __init__(self, args, train_platform, model, diffusion, data):
        ...
        if self.resume_step:
            print(f"### Resuming from checkpoint {self.resume_checkpoint} at step {self.resume_step} ...")
    ...
    def mix_during_training(self):
        if not self.args.gen_during_training:
            return
        mix_args = copy.deepcopy(self.args)
        mix_args.model_path = os.path.join(self.save_dir, self.ckpt_file_name())
        mix_args.output_dir = os.path.join(self.save_dir, f'{self.ckpt_file_name()}.samples')
        mix_args.num_samples = self.args.gen_num_samples
        mix_args.num_repetitions = self.args.gen_num_repetitions
        mix_args.motion_length = 6.0 #None  # length is taken from the dataset
        mix_args.load_from_model_name = True
        mix_args.alpha = self.args.gen_alpha

        all_objects = self.data.dataset.motion_dataset.cond_dict.keys() 
        random.seed(self.step)
        mix_args.object_type = random.sample(all_objects, mix_args.num_samples)
        mix_args.mix_samples = []

        random.seed(self.args.seed)
        all_sample_save_path = mix(mix_args, self.data.dataset.motion_dataset.cond_dict)
        self.train_platform.report_media(title='Motion', series='Predicted Motion', iteration=self.total_step(),
                                         local_path=all_sample_save_path)
    ...
    def save(self):
            def save_checkpoint():
                ...
                def del_base_model(state_dict):
                    # don't save save pre-trained AnyTop weights (in case of MMix)
                    if self.args.control:
                        pretrained_weights = [
                            e for e in state_dict.keys() if e.startswith('base_model.')
                        ]
                        for e in pretrained_weights:
                            del state_dict[e] 
